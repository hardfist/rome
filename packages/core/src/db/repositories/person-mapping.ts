...
export class PersonMappingRepository {
  ...
  /**
   * File an identity under the stranger sentinel, unless a person holds it.
   *
   * The conditional half of {@link writeChannelMapping}: that upsert re-points
   * whoever holds the identity, which is right for a channel reporting who its
   * account is now and wrong for a dismissal. A dismissal decides about an
   * account nobody has placed, so a placement arriving between the caller's
   * check and this write has to win — the check cannot see it, and re-pointing
   * would take a person the guardian placed and file them under the sentinel,
   * silently. The condition is the same one {@link writeReleaseStrangerClaim}
   * scopes to, for the same reason: only a claim of the sentinel's own is the
   * sentinel's to move.
   *
   * A row already the sentinel's yields, so dismissing twice is one dismissal
   * rather than a refusal.
   */
  writeClaimForStranger(exec: SqliteExec, channel: string, channelUserId: string): boolean {
    const row = exec
      .insert(channelMappings)
      .values({ id: uuid(), personId: STRANGER_PERSON_ID, channel, channelUserId })
      .onConflictDoUpdate({
        target: [channelMappings.channel, channelMappings.channelUserId],
        set: { personId: STRANGER_PERSON_ID },
        // The stored name is the channel's and stays put: a dismissal is news
        // about the guardian's intent, not about what the platform calls the
        // account.
        setWhere: eq(channelMappings.personId, STRANGER_PERSON_ID),
      })
      .returning({ id: channelMappings.id })
      .get();
    // Nothing returned means the conflicting row failed that condition, which
    // is to say a person holds the identity.
    return row != null;
  }
